...
class convolution:
	def __init__(self, count, height, width):
		self.count = count
		self.height = height
		self.width = width
...
class NN:
	def __init__(self, layers):
		...
		self.matrices = []
		for layer in layers:
			if layer.__class__ == convolution:
				self.matrices.append([])
				for i in range(layer.count):
					self.matrices[-1].append([[uniform(-1, 1) for j in range(layer.width)] for i in range(layer.height)])
			elif layer.__class__== full_connected:
				self.k = layer.size

		self.w = [[[uniform(-1, 1) for l in range(self.k[i + 1])] for j in range(self.k[i])] for i in range(len(self.k) - 1)]
		self.delta_w = [[[0 for l in range(self.k[i + 1])] for j in range(self.k[i] + 1)] for i in range(len(self.k) - 1)]

		self.bias = [[uniform(-1, 1) for j in range(self.k[i])] for i in range(len(self.k) - 1)]
		self.delta_bias = [[0 for j in range(self.k[i])] for i in range(len(self.k) - 1)]
	...
	def backpropagation(self, data_output, n=0.7, m=0.5):
	...
	def convolution(self, image, matrix, bias, stride_h=None, stride_w=None):
		new_image = []
